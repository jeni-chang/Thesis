...
public class PutCPHead {
	...
	private List<Integer> cp_tmp = new ArrayList<>();
	private double f; 
	
	public PutCPHead( int layer, List<Double> r, List<Double> pb, List<Double> lc, List<Double> cc, List<Double> bw, List<Double> com, Double f) {
		this.layer = layer;
		this.r = r;
		this.pb =pb;
		this.lc = lc;
	...
	public double compute(int cp_num, int server) {
		...
		// generate server from head to compute
		for(int i=1; i<=cp_layer.size(); i++) cp_server.add(i);
